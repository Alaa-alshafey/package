        ...
        <table class="table datatable-button-init-basic">
            ...
            @foreach($banners as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->title_ar}}</td>
                    <td>{{$item->title_en}}</td>
                    <td>{{$item->banner_type == "ads"? "بانر للاعلانات" : "بانر عام"}}</td>
                    <td>{{$item->end_date}}</td>
                    {!!Form::open( ['route' => ['admin.banners.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('admin.banners.edit',$item->id)}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
