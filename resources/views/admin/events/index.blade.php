...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">كل البطاقات</h5>
        ...
        <div class="panel-body">
            عرض كل البطاقات والتحكم بهم وبكل العمليات الخاصة بهم مع امكانية البحث وتصدير تقارير وملفات وطباعتهم
        </div>
        ...
        <table class="table datatable-button-init-basic">
            ...
            @foreach($events as $key=>$item)
                <tr>
                    <td>{{$key+1}}</td>
                    <td>{{$item->title}}</td>
                    <td><img src="{{getimg($item->image)}}" style="width: 250px; height: 250px" class="img-responsive img-fluid"></td>
                    <td>{{($item->SubCat->title)}}</td>
                    {!!Form::open( ['route' => ['admin.events.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('admin.events.edit',$item->id)}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
        ...
        function Delete(id) {
            ...
            swal({
                ...
                if(isConfirm){
                    ...
                    swal("تم االإلفاء", "حذف البطاقة تم الغاؤه",'info',{buttons:'موافق'});
                }
            });
        }
    </script>
