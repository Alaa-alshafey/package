...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">كل الخدمات</h5>
        ...
        <div class="panel-body">
            عرض كل الخدمات   والتحكم بهم وبكل العمليات الخاصة بهم مع امكانية البحث وتصدير تقارير وملفات وطباعتهم
        </div>
        ...
        <table class="table datatable-button-init-basic">
            ...
            @foreach($services as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->ar_name}}</td>
                    <td>{{$item->en_name}}</td>
                    {!!Form::open( ['route' => ['admin.service.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('admin.service.edit',['id'=>$item->id])}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
