...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">كل المؤهلات</h5>
        ...
        <div class="panel-body">
            عرض كل المؤهلات والتحكم بهم وبكل العمليات الخاصة بهم مع امكانية البحث وتصدير تقارير وملفات وطباعتهم
        </div>
        ...
        <table class="table datatable-button-init-basic">
            ...
            @foreach($qualification as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->ar_name}}</td>
                    <td>{{$item->en_name}}</td>
                    {!!Form::open( ['route' => ['admin.qualification.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('admin.qualification.edit',$item->id)}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
        ...
        function Delete(id) {
            ...
            swal({
                ...
                if(isConfirm){
                    ...
                    swal("تم االإلفاء", "حذف المؤهل تم الغاؤه",'info',{buttons:'موافق'});
                }
            });
        }
    </script>
