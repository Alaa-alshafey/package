...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">كل الدول</h5>
        ...
        <table class="table datatable-button-init-basic">
            ...
            @foreach($country as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->ar_name}}</td>
                    <td>{{$item->en_name}}</td>
                    {!!Form::open( ['route' => ['admin.country.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('admin.country.edit',$item->id)}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
    ...
    <script>
        function Delete(id) {
            ...
            swal({
                title: "هل أنت متأكد ",
                text: "هل تريد حذف هذه الدولة ؟ سوف يتم حذف جميع المدن التابعه لها بالإضافة للمستخدمين",
                icon: "warning",
                buttons: ["الغاء", "موافق"],
                dangerMode: true,
