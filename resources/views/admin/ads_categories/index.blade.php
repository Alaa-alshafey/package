...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">كل أقسام الاعلانات</h5>
        ...
        <div class="panel-body">
            عرض كل أقسام الاعلانات والتحكم بهم وبكل العمليات الخاصة بهم مع امكانية البحث وتصدير تقارير وملفات وطباعتهم
        </div>
        ...
        <table class="table datatable-button-init-basic">
            ...
            @foreach($categories as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->ar_name}}</td>
                    <td>{{$item->en_name}}</td>
                    {!!Form::open( ['route' => ['admin.adscategories.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('admin.adscategories.edit',$item->id)}}"
